...
function AuthPage() {
  ...
  return (
    <>
      <AuthShell
        badge={tab === "signin" ? "Returning operator" : "New account"}
        title={tab === "signin" ? "Sign back in" : "Join the network"}
        subtitle={tab === "signin" ? "Resume your watch over Michigan weather." : "Create a free account to save locations and enable alerts."}
      >
        <div className="grid gap-2">
          <Button type="button" variant="outline" onClick={() => signInOAuth("google")} disabled={busy} className="w-full h-11 border-border/80">
          ...
          <Button type="button" variant="outline" onClick={() => signInOAuth("apple")} disabled={busy} className="w-full h-11 border-border/80">
        ...
        <div className="relative my-1">
          ...
          <div className="relative flex justify-center"><span className="bg-card px-3 text-[10px] font-mono uppercase tracking-[0.25em] text-muted-foreground">or with email</span></div>
        </div>
          ...
          <form onSubmit={submit} className="space-y-3 pt-4">
            <TabsContent value="signup" className="space-y-3 m-0">
              <Field icon={User} label="Display name">
                <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Doe" className="pl-9 h-11" />
              </Field>
            </TabsContent>

            <Field icon={Mail} label="Email">
              <Input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="pl-9 h-11" />
            </Field>

            <Field icon={Lock} label="Password">
              <Input type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="pl-9 h-11" />
            </Field>

            <Button type="submit" disabled={busy} className="w-full h-11 font-display tracking-wider mt-2">
